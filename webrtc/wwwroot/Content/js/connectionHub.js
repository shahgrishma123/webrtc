...
const callbackUserMediaSuccess = (stream) => {
    ...
    otherVideo = document.querySelector('.video.mine');
    //attachMediaStream(otherVideo, evt.stream); // from adapter.js

    attachMediaStream(otherVideo,stream);
};
...
const callbackAddStream = (connection, evt) => {
    ...
    // Bind the remote stream to the partner window
     otherVideo = document.querySelector('.video.partner');
    //attachMediaStream(otherVideo, evt.stream); // from adapter.js

    attachMediaStream(otherVideo,evt.stream);
}
